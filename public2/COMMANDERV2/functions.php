...
function addLearnedBot($ip, $userAgent, $reason, $campaignId = '') {
    ...
    // Limita tamanho (max 50k fingerprints, 20k IPs)
    if (count($learnedBots['fingerprints']) > 50000) {
        $learnedBots['fingerprints'] = array_slice($learnedBots['fingerprints'], -40000, null, true);
    }
    if (count($learnedBots['ips']) > 20000) {
        $learnedBots['ips'] = array_slice($learnedBots['ips'], -15000, null, true);
    }
